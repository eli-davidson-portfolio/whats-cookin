...
describe("Pantry", () => {
  ...
  beforeEach(() => {
    rr = new RecipeRepository(
      recipesTestData.recipesTestData,
      ingredientsData.ingredientsData
    );
    testRecipe = rr.recipes[0];
    user = new User({
      name: "Saige O'Kon",
      id: 1,
      pantry: [
  ...
  it("Should store an array of ingredients that equals expected ingredient", () => {
    ...
    expect(user.pantry.ingredients[0]).to.deep.equal({
      ingredient: 11297,
      amount: 4,
    });
  });
  ...
  it("Should determine if a user can cook a recipe", () => {
    ...
    ingredientsArray.push(
      new Ingredient(20081, "wheat flour", 142, "serving", 1)
    );
    ingredientsArray.push(
      new Ingredient(18372, "bicarbonate of soda", 582, "serving", 1)
    );
    ingredientsArray.push(new Ingredient(1123, "eggs", 472, "serving", 1));
    user.fillPantry(ingredientsArray);
    expect(user.checkIngredients(ingredientsArray)).to.deep.equal([
      {
        amount: 0,
        id: 20081,
        name: "wheat flour",
